...
CAEmitterLayer *getEmitterForImageInFrame(CGImageRef sparkleImage, CGSize imageSize) {
    ...
    sparkle.contents = (__bridge id) (sparkleImage);
...
void animateImage() {
    ...
    // Gather image paths
    NSString *folder = [NSHomeDirectory() stringByAppendingPathComponent:@".rainicornleap"];

    NSString *imageName = @"";
    NSString *audioName = @"";
    if (lemongrab) {
        imageName = @"Earl.png";
        audioName = @"unacceptable.mp3";
    } else {
        imageName = @"raincorn.png";
